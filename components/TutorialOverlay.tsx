    ...
    <div className="fixed inset-0 z-[100] pointer-events-auto">
      ...
      {targetRect ? (
        <div className="absolute inset-0">
          {/* Top */}
          <div
            className="absolute top-0 left-0 right-0 bg-black/80 backdrop-blur-[1px] transition-all duration-500"
            style={{ height: targetRect.top - 8 }}
          ></div>
          {/* Bottom */}
          <div
            className="absolute bottom-0 left-0 right-0 bg-black/80 backdrop-blur-[1px] transition-all duration-500"
            style={{ top: targetRect.bottom + 8 }}
          ></div>
          {/* Left */}
          <div
            className="absolute left-0 bg-black/80 backdrop-blur-[1px] transition-all duration-500"
            style={{
          ...
          {/* Right */}
          <div
            className="absolute right-0 bg-black/80 backdrop-blur-[1px] transition-all duration-500"
            style={{
              top: targetRect.top - 8,
              bottom: window.innerHeight - (targetRect.bottom + 8),
        ...
        style={{
          top: targetRect
            ? Math.min(
                ...
                Math.max(20, targetRect.bottom + 24),
              )
            : "50%",
          left: targetRect
            ? Math.max(20, Math.min(window.innerWidth - 340, targetRect.left))
            : "50%",
          transform: targetRect ? "none" : "translate(-50%, -50%)",
        }}
      >
        <div className="bg-canvas border border-line rounded-xl shadow-2xl p-6 relative overflow-hidden">
          {/* Decorative background */}
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-500 to-emerald-500"></div>

          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-2">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-indigo-600 text-ink text-xs font-bold">
                {currentStepIndex + 1}
              </span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1">
              {Array.from({ length: totalSteps }).map((_, idx) => (
                <div
                  key={idx}
                  className={`w-1.5 h-1.5 rounded-full ${idx === currentStepIndex ? "bg-indigo-500" : "bg-inset"}`}
                ></div>
              ))}
            </div>
