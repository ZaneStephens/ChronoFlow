...
const RockManager: React.FC<RockManagerProps> = ({ 
  ...
  const handleDeleteRock = () => {
      ...
      if (window.confirm("Are you sure you want to delete this Rock? This cannot be undone.")) {
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 relative overflow-hidden shadow-2xl">
                ...
                <div className="flex justify-between items-start relative z-10 mb-6">
                    <div>
                        ...
                        <h1 className="text-3xl md:text-4xl font-bold text-white mb-4 leading-tight">{selectedRock.title}</h1>
                    ...
                    <div className="h-3 bg-slate-700/50 rounded-full overflow-hidden backdrop-blur-sm">
                        <div 
                            className={`h-full transition-all duration-1000 ${
                                selectedRock.status === 'off-track' ? 'bg-red-500' : 
                 ...
                 <div className="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden divide-y divide-slate-700/50">
                     {krs.map((kr) => (
                         <div 
                            ...
                            className={`group p-5 flex items-start gap-4 hover:bg-slate-700/30 transition-colors cursor-pointer ${kr.isCompleted ? 'bg-slate-800/80' : ''}`}
                         >
                             <div className={`mt-0.5 transition-colors ${kr.isCompleted ? 'text-emerald-500' : 'text-slate-500 group-hover:text-slate-300'}`}>
                  ...
                  return (
                      ...
                      >
                          <div className={`absolute top-0 left-0 w-1 h-full ${
                              rock.status === 'off-track' ? 'bg-red-500' : 
