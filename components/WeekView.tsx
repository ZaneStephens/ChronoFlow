...
const WeekView: React.FC<WeekViewProps> = ({
  ...
  const weekPercentage = Math.min(
    (weekTotalSeconds / weekGoalSeconds) * 100,
    100,
  );
  const daysMetGoal = weekData.filter(
    (d) => d.totalSeconds >= DAILY_GOAL_SECONDS,
  ).length;
  ...
  const getHeatColor = (seconds: number): string => {
    if (seconds === 0) return "bg-surface";
        ...
        <div className="bg-surface border border-line rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-semibold text-ink">Week Overview</h3>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setWeekOffset((prev) => prev - 1)}
                className="p-1.5 hover:bg-inset rounded text-muted hover:text-ink transition-colors"
              >
                <ChevronLeft size={18} />
              </button>
              <button
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-canvas rounded-lg p-3 border border-line/50">
              <p className="text-[10px] uppercase tracking-wider text-quiet font-bold mb-1">
                Week Total
              </p>
              ...
              <p className="text-xs text-quiet">
                {weekPercentage.toFixed(0)}% of{" "}
                {formatDuration(weekGoalSeconds)} goal
              </p>
            </div>
            <div className="bg-canvas rounded-lg p-3 border border-line/50">
              <p className="text-[10px] uppercase tracking-wider text-quiet font-bold mb-1">
              ...
              <p className="text-xl font-bold text-ink">
                {daysMetGoal} <span className="text-sm text-quiet">/ 5</span>
              </p>
              <p className="text-xs text-quiet">working days</p>
            </div>
            <div className="bg-canvas rounded-lg p-3 border border-line/50">
              ...
              <p className="text-xl font-bold text-ink">
                {currentStreak} <span className="text-sm text-quiet">days</span>
              </p>
              <p className="text-xs text-quiet">50%+ daily goal</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-7 gap-2">
            {weekData.map(
              ({ date, totalSeconds, percentage, isToday, isFuture }) => {
                const dayLabel = date.toLocaleDateString("en-AU", {
                  weekday: "short",
                });
                ...
                return (
                  ...
                  >
                    <div className="text-[10px] font-bold uppercase tracking-wider text-quiet">
                      {dayLabel}
                    </div>
                    <div
                      className={`text-sm font-bold ${isToday ? "text-indigo-400" : "text-body"}`}
                    >
                      {dateLabel}
                    </div>
                    {/* Vertical bar */}
                    <div className="w-full h-24 bg-canvas rounded-md overflow-hidden flex flex-col-reverse border border-line/50">
                      <div
          ...
          {weekClientBreakdown.length > 0 && (
            <div className="mt-6 pt-4 border-t border-line/50">
              <p className="text-xs uppercase tracking-wider text-quiet font-bold mb-3">
                Client Distribution
              </p>
              <div className="flex gap-1 h-3 rounded-full overflow-hidden bg-canvas">
      ...
      {showHeatmap && (
        <div className="bg-surface border border-line rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrevHeatmapMonth}
                className="p-1.5 hover:bg-inset rounded text-muted hover:text-ink transition-colors"
              >
                <ChevronLeft size={18} />
              </button>
              <span className="text-sm font-medium text-body min-w-[130px] text-center">
                {monthName}
              </span>
              <button
                onClick={handleNextHeatmapMonth}
                className="p-1.5 hover:bg-inset rounded text-muted hover:text-ink transition-colors"
              >
                <ChevronRight size={18} />
              </button>
          ...
          <div className="grid grid-cols-7 gap-1.5 mb-1.5">
            {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
              <div
                key={d}
                className="text-center text-[10px] font-bold uppercase tracking-wider text-quiet"
              >
                {d}
              </div>
            ))}
              ...
              return (
                <div
                  ...
                  className={`aspect-square rounded-md flex items-center justify-center relative group cursor-default transition-all ${getHeatColor(day.seconds)} ${
                    isToday
                      ? "ring-2 ring-indigo-400 ring-offset-1 ring-offset-slate-800"
                      : ""
                  } hover:ring-1 hover:ring-slate-500`}
                >
                  <span
                    className={`text-xs font-medium ${day.seconds > 0 ? "text-ink/80" : "text-quiet"}`}
                  >
                    {day.date.getDate()}
                  </span>

                  {/* Tooltip */}
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 bg-canvas border border-line rounded-lg shadow-xl text-xs text-ink opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
          ...
          <div className="flex items-center justify-end gap-2 mt-4">
            <span className="text-[10px] text-quiet">Less</span>
            ...
            <span className="text-[10px] text-quiet">More</span>
          </div>
        </div>
      )}
    </div>
